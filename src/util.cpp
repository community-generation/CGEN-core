...
int nAnonymizeCGENcoreAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cgencore" is a composite category enabling all CGEN-related debug output
            if (ptrCategory->count(string("cgencore"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CGEN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CGEN
// Mac: ~/Library/Application Support/CGEN
// Unix: ~/.cgencore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CGEN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cgencore.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cgencore.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cgencored.pid"));
